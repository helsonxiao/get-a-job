...
def build_resume_prompt(
    ...
    *,
    ...
    sections.append(
        ...
        "6. **证据分级**: 简历中的每条事实按证据强度分 A/B/C/D 四级——A=有代码/日志/\n"
        "   测试等可直接证明, B=合理推断但须标注待确认, C=仅有当事人记忆, D=证据冲突\n"
        "   或夸大。**A/B 级才能写入, C 级只能定性描述, D 级一律删除**。\n"
        "7. **量化门禁**: 写数字前必须通过四道检查——①命令/测试/基准可复现; ②有有效\n"
        "   基线对比(不是孤立绝对数); ③成果可归属本人行动; ④来源与算法有记录。\n"
        "   任一不满足 → 改用定性描述, 宁缺毋滥。\n"
        "8. **敏感措辞泛化**: 涉及小众/敏感社群属性的项目名、平台黑话, 一律泛化为通用\n"
        "   领域描述(如\"XX 平台数据看板\"), 保留技术含量、抹去可能引发偏见的指向。"
    )
