...
## 6b. 列表级口径重归属（多口径同岗位自动挪动）

同一岗位被多个筛选链接命中时, 爬虫按 job_id 跳过重复详情抓取 ——
为此采集完成后会自动执行**列表级重归属**: 凡出现在本次筛选列表中的岗位
（含被跳过详情的历史岗位）, 口径统一改为本次链接。

- 新口径采集后, 历史岗位自动挪过来, 无需手工操作;
- 同岗位只属一个口径（单值归属, 最近采集优先）, 合并展示靠 job_id 主键天然去重;
- 日志: `列表级口径重归属: 列表出现 N 岗, 重归属 M 岗`。

## 7. Web 全局口径筛选

侧边栏顶部「数据口径」下拉 = 全局筛选：选择某口径后，职位列表 / 市场观察台 / 公司图鉴 /
统计等全部只读视图自动按该口径过滤（前端请求自动附加 `?scope=`，后端 temp 影子生效）；
选「全部数据」恢复。选择持久化在浏览器 localStorage。

「口径管理」页支持：长链接完整多行显示、重命名（报告标题用）、「设为当前口径」、
未分口径历史数据报数。

**安全边界**：口径影子只对 GET 只读接口生效 —— 收藏/忽略/调分/删除等写操作
永远作用于全库，不会写进影子表丢失。

## 8. 历史数据归集记录

2026-08-31 已将库内全部 364 条（可见 269）历史数据统一归集到主口径：
「无锡-全栈工程师-主口径」（即本节提供的全栈筛选链接），未分口径归零。
后续如需拆分更窄口径，用 `gaj scope-link assign` 或重新采集。
